...
public class CheckBox extends CustomView {
	...
	protected void setAttributes(AttributeSet attrs) {
        ...
        if(text != null) {
			...
			textView.setTextColor(getResources().getColor(R.color.material_prime_blue));
			...
			if (textColor != -1) {
				...
				// Color by hexadecimal
				// Color by hexadecimal
				int background = attrs.getAttributeIntValue(ANDROIDXML, "textColor", -1);
				if (background != -1)
					textView.setTextColor(background);
			}
			/*
			int textSize = attrs.getAttributeResourceValue(ANDROIDXML,
																 "textSize", -1);
			if (bacgroundColor != -1) {
				textView.setTexts
			} else {
		...
		protected void onDraw(Canvas canvas) {
			...
			if (check) {
				if (step < 11){
					step++;
					invalidate();
					//Log.v("MyView","orginal check: "+check);
				}
			} else {
